...
const QString BITCOIN_IPC_PREFIX("populacecoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click populacecoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start populacecoin: click-to-pay handler");
    else
...
{
    // clicking on populacecoin: URLs creates FileOpen events on the Mac:
