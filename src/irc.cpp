...
// Copyright (c) 2009-2012 The POPULACECOIN developers
...
bool RecvLineFixed(SOCKET hSocket, string& strLine)
{
    ...
    while (true)
    {
        ...
        // +POPULACECOIN added for irc exits, without this, recv(hSocket, &c, 1, 0); will hang when closing the program after using irc on windows.
		bool stop = false;
		u_long RecvCount;
        while(!stop)
        {
...
void ThreadIRCSeed()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            ...
            Send(hSocket, strprintf("JOIN #POPULACECOIN20%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #POPULACECOIN20%02d\r", channel_number).c_str());
        }
