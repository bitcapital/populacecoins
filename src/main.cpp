...
// Copyright (c) 2013 The Populacecoin developers
...
uint256 hashGenesisBlock("0x000000b8ec60563518002b9efd2da0df9cdd3e9ef76e4e64646a3572f3810ca7");
static const unsigned int timeGenesisBlock = 1394064000;//Human time (GMT): Thu, 06 Mar 2014 00:00:00 GMT
...
const string strMessageMagic = "Populacecoin Signed Message:\n";
...
static const int64 nTargetTimespan = 120 * 30; // 60 minutes, 120 blocks
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    else if(nHeight<=51840)
    {
        nSubsidy=10240*COIN;
    }
    else if(nHeight<=155520)
    {
        nSubsidy=5120*COIN;
    }
    else if(nHeight<=466560)
    {
        nSubsidy=2560*COIN;
    }
    else if(nHeight<=1399680)
    {
        nSubsidy=1280*COIN;
    }
    else
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "POC - PopulaceCoin - popular over the world.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xb57ee33e30939a0ca9eeba41b8c7e423fe5eff87561277ce36583617a7095ff7"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PopulacecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PopulacecoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PopulacecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("populacecoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running PopulacecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("PopulacecoinMiner terminated\n");
        throw;
    }
}
